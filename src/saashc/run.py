...
def preprocess_arguments():
    ...
    global instance_name
    if instance_name and not instance_name.endswith(".thop"):
        instance_name += ".thop"
    ...
    if executable_path is None:
        executable_path = src_dir / f"acothop{postfix}.exe"
    else:
    ...
    global sol_dir
    sol_dir = Path(sol_dir) if sol_dir else src_dir.parent / "solutions/temp/temp"
...
def build():
    # Windows-only build using Visual Studio
    ...
    exe_src = build_dir / build_type / "acothop.exe"
    exe_dst = Path(executable_path)
...
def format_aco_command():
    ...
    if random_seed:
        command += ["--seed", str(random_seed)]
    if ants:
        command += ["--ants", str(ants)]
    if alpha:
    ...
    if rho:
        command += ["--rho", str(rho)]
    if ptries:
        command += ["--ptries", str(ptries)]
    if localsearch:
        command += ["--localsearch", str(localsearch)]

    if not no_log:
        command += ["--outputfile", str(output_path), "--log"]
