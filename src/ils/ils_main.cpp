...
int main(int argc, char* argv[]) {
    ...
    cerr << "Instance: " << data.problemName << endl;
    using namespace std::chrono;
    high_resolution_clock::time_point t1 = high_resolution_clock::now();
    cerr << "Seed: " << rngSeed << endl;

    rng = MTRand(rngSeed);				// initialize the random number generator       

    cerr << "Running iterated local search..." << endl;
    Solution solution = iteratedLocalSearch(runtime, solutionFile + ".log");
    cerr << "Iterated local search done" << endl;

    high_resolution_clock::time_point t2 = high_resolution_clock::now();
    duration<double> time_span = duration_cast<duration<double> >(t2 - t1);

    cerr << "Total time: " << time_span.count() << " seconds." << endl;
        
    solution.evaluate(solutionFile);

    cerr << "Solution written to " << solutionFile << endl;
